...
/**
 * Mismo glifo oficial que WhatsAppIcon en button.tsx -- se duplica aquí
 * (viewBox propio, 0 0 32 32) en vez de exportarlo desde ahí porque ese
 * archivo lo trata como un detalle interno del botón, no como parte de la
 * API pública de íconos.
 *
 * `fill` fijo en `--color-whatsapp`, no `currentColor` -- en `ShareButtons`
 * este ícono vive dentro de un botón circular `color: var(--color-ink)`
 * (fondo arena), así que heredaba negro/tinta y se perdía contra el resto
 * de la fila (Svei lo reportó en /tours). El verde de WhatsApp está
 * reservado en HANDOFF §7 "solo para el CTA de WhatsApp" -- un ícono de
 * compartir por WhatsApp es exactamente ese caso, no una excepción.
 */
export function WhatsappIcon({ size = 20 }: IconProps) {
  return (
    <svg viewBox="0 0 32 32" width={size} height={size} fill="var(--color-whatsapp)" aria-hidden="true">
